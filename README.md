...
این پروژه، پیاده‌سازی یک دستیار هوشمند مکالمه‌ای برای یک اپلیکیشن فرضی سفارش غذاست. این سیستم که با استفاده از **LangGraph** و معماری **چند-ایجتی (Multi-Agent)** ساخته شده و قابلیت‌های پیشرفته‌ای را برای ارائه یک تجربه کاربری هوشمند، سریع و کاملاً تعاملی فراهم می‌کند.
